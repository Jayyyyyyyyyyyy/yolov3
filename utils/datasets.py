...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, single_channel=False):
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            if self.single_channel:
                img0 = cv2.imread(path, 0) # gray single channel
        ...
        # cv2.imwrite('./letterbox_bug.jpg', img)
        # Convert
        if self.single_channel:
            img = img[:, :, np.newaxis]
...
class LoadImagesAndLabels(Dataset):
    ...
    def  __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if cache_images:
            ...
            results = ThreadPool(NUM_THREADS).imap(lambda x: load_image(*x, self.hyp['single_channel']), zip(repeat(self), range(n)))
...
# Ancillary functions --------------------------------------------------------------------------------------------------
def load_image(self, i, single_channel=False):
    ...
    if im is None:  # not cached in ram
        ...
        if npy and npy.exists():  # load npy
            ...
            if single_channel:
                im = cv2.imread(path, 0) # gray single channel
                im = im[:, :, np.newaxis]
            else:
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w) = load_image(self, index, self.hyp['single_channel'])
